local utils = require "php-lsp-utils.utils"
...
local function go_to(client, is_new_instance, full_class, method)
    ...
    if vim.tbl_isempty(locations) then
        vim.notify("Empty reponse loogin for class" .. full_class, vim.log.levels.WARN, {})
    ...
    if class_location == nil then
        vim.notify("Could not find class for : " .. full_class, vim.log.levels.WARN, {})
        if is_new_instance then
            vim.lsp.stop_client(client.id)
        end
        return
    ...
    utils.open_filename(class_location.filename)
    ...
    vim.lsp.buf_request(0, "textDocument/documentSymbol", params, function(method_err, method_server_result, _, _)
        ...
        if row and col then
            ...
            if not ok then
                vim.notify("Erro setting row and col " .. err_msg, vim.log.levels.WARN, {})
            end
            vim.cmd "normal zt"
        end
        if is_new_instance then
